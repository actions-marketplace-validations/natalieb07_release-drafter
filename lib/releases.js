...
module.exports.findReleases = async ({ app, context }) => {
  ...
  const sortedPublishedAllReleases = sortReleases(releases.filter((r) => !r.draft))
  log({ app, context, message: `Found ${sortedPublishedAllReleases.length} all published releases` })
  const sortedPublishedReleases = sortedPublishedAllReleases(sortedPublishedAllReleases.filter((r) => !r.prerelease))
  log({ app, context, message: `Found ${sortedPublishedReleases.length} published releases that are not prereleases` })
